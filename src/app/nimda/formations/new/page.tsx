...
export default function NewFormationPage() {
  ...
  const [form, setForm] = useState<any>({
    ...
    garantie_remboursement_jours: 0,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (created?.id_formation) {
        router.push(`/nimda/formations/${created.id_formation}`)
      } else {
    ...
    <div className="p-6 space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2">
              <FormationImageUpload
                ...
                onChange={(url) => update('image_couverture_url', url)}
              />
            </div>
            <div>
              <Label>Image de couverture URL (manuel)</Label>
              <Input value={form.image_couverture_url} onChange={(e) => update('image_couverture_url', e.target.value)} />
            </div>
            <div>
            ...
            <div>
              ...
              <Input value={form.url_slug} onChange={(e) => update('url_slug', e.target.value)} placeholder="auto à partir du titre si vide" />
            </div>
          </div>
        </Card>
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          <Button type="submit" disabled={saving} className="bg-blue-600 hover:bg-blue-700">{saving ? 'Enregistrement...' : 'Enregistrer'}</Button>
        </div>
      </form>
    </div>
  )
